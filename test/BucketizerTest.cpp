...
using AllocatorUnderTest = alb::bucketizer<
    alb::shared_freelist<
        alb::mallocator,
        static_cast<std::size_t>(alb::internal::dynastic_options::dynastic_dynamic_set),
        static_cast<std::size_t>(alb::internal::dynastic_options::dynastic_dynamic_set)>,
    17,
    64,
    16>;
...
TEST_F(BucketizerTest, ThatMinAndMaxSizeOfTheBucketItemsAreSetSpecifiedByTheParameter) {
    ...
    EXPECT_EQ(17u, sut.buckets_[0].min_size());
    EXPECT_EQ(32u, sut.buckets_[0].max_size());

    EXPECT_EQ(33u, sut.buckets_[1].min_size());
    EXPECT_EQ(48u, sut.buckets_[1].max_size());

    EXPECT_EQ(49u, sut.buckets_[2].min_size());
    EXPECT_EQ(64u, sut.buckets_[2].max_size());
}
...
TEST_F(BucketizerTest,
       ThatAReallocationWithInTheEdgesOfABucketItemTheLengthAndTheContentIsTheSame) {
