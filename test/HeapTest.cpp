...
template <class T>
class HeapWithSmallAllocationsTest : 
  public AllocatorBaseTest<T>
{
};
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatItsPossibleToAllocateAllMemoryBlocksAndThatAllOfThemAreContiguous)
{
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatANullBlockIsReturnedIfABiggerChunkIsReqeustedThanTheCompleteHeapCanHandle)
{
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatItsPossibleToAllocateHalfNumberOfChunksMemoryBlocksWithDoubleBlockSizeBytesAndThatAllOfThemAreContiguous)
{
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatASmallerFreedBlockIsNotUsedForANewAllocationButANewContiguousBlockIsUsed)
{
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatExpandByZeroBytesOfAnEmptyBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(0);
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatExpandByZeroBytesOfAFilledBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(SmallChunkSize);
  auto origMem = mem;
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatExpandByNBytesOfAFilledBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(SmallChunkSize);
  auto origMem = mem;
  fillBlockWithReferenceData<int>(mem);
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatReallocatrByZeroBytesOfAnEmptyBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(0);
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatReallocateByZeroBytesOfAFilledBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(SmallChunkSize);
  auto origMem = mem;
  fillBlockWithReferenceData<int>(mem);
...
TYPED_TEST(HeapWithSmallAllocationsTest, ThatReallocateByNBytesOfAFilledBlockReturnsSuccessAndDoesNotChangeTheProvidedBlock)
{
  auto mem = this->sut.allocate(SmallChunkSize);
  auto origMem = mem;
  fillBlockWithReferenceData<int>(mem);
...
template <class T>
class HeapWithLargeAllocationsTest : public 
  AllocatorBaseTest<T>
{
};

typedef ::testing::Types<
  alb::shared_heap<alb::mallocator, 512, 8>,
  alb::heap<alb::mallocator, 512, 8>
> TypesForLargeHeapTest;
...
TYPED_TEST(HeapWithLargeAllocationsTest, ThatAllocatingSeveralWholeChunksAfterASingleBlockIsSupported)
{
...
TYPED_TEST(HeapWithLargeAllocationsTest, ThatWhenAllChunksAreUsedNoMemoryCanBeAllocatedAndThatDeallocateAllFreesMakesAllBlocksAgainAvailable)
{
  alb::block blocks[8];
  for (auto& b : blocks) { b = this->sut.allocate(64*8); }
...
TYPED_TEST(HeapWithLargeAllocationsTest, ThatAllocatingMemoryBiggerThanAChunkSizeWorksAfterASingleMemoryBlockHasTheCorrectOffsets)
{
