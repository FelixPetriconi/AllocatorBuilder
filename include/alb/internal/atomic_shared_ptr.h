...
public:
    ...
    bool is_lock_free() const noexcept { return std::atomic_is_lock_free(); }

    void store(std::shared_ptr<T> p, std::memory_order order = std::memory_order_seq_cst) noexcept {
    ...
    bool compare_exchange_strong(std::shared_ptr<T>& old_value,
    ...
    bool compare_exchange_weak(std::shared_ptr<T>& old_value,
                               std::shared_ptr<T> new_value,
                               std::memory_order order = std::memory_order_seq_cst) noexcept;
