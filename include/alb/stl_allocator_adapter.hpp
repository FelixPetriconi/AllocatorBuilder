...
public:
    ...
    explicit std_allocator_adapter(const Allocator& allocator) noexcept : allocator_(allocator) {}
    ...
    void construct(pointer ptr) { ::new (static_cast<void*>(ptr)) T; };
...
bool operator==(const std_allocator_adapter<T1, Allocator>&,
                const std_allocator_adapter<T2, Allocator>&) {
    return true;
}

template <class Allocator, typename T1, typename T2>
bool operator!=(const std_allocator_adapter<T1, Allocator>& x,
