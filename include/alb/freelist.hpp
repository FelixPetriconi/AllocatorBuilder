...
public:
     ...
     */
    size_t min_size() const noexcept { return _lowerBound.value(); }

    /**
     * Returns the upper boundary
     */
    size_t max_size() const noexcept { return _upperBound.value(); }
    ...
    block allocate(size_t n) noexcept {
        ...
        assert(_lowerBound.value() !=
               ::std::numeric_limits<size_t>::max()); // "The lower bound was not initialized!");
        assert(_upperBound.value() !=
               ::std::numeric_limits<size_t>::max()); // "The upper bound was not initialized!");
        ...
        if (_lowerBound.value() <= n && n <= _upperBound.value()) {
            ...
            if (root_.pop(freeBlock)) {
                ...
                result.length = _upperBound.value();
                return result;
            }
            ...
            if (supports_truncated_deallocation) {
                ...
                if (batchAllocatedBlocks) {
                    ...
                    for (size_t i = 1; i < NumberOfBatchAllocations; i++) {
                        if (!root_.push(static_cast<char*>(batchAllocatedBlocks.ptr) +
                                        i * blockSize)) {
                            assert(false);
                    ...
                    result.ptr = batchAllocatedBlocks.ptr;
                    result.length = blockSize;
                    return result;
                }
                result = allocator_.allocate(blockSize);
                return result;
            }
            for (size_t i = 0; i < NumberOfBatchAllocations - 1; i++) {
                result = allocator_.allocate(blockSize);
                if (!root_.push(result.ptr)) { // the list is full in the meantime, so we
                                               // exit early
                    return result;
                }
            }
...
class shared_freelist
    : public freelist_base<true, Allocator, MinSize, MaxSize, PoolSize, NumberOfBatchAllocations> {
public:
...
template <class Allocator,
          ...
          size_t NumberOfBatchAllocations = 8>
class freelist
    : public freelist_base<false, Allocator, MinSize, MaxSize, PoolSize, NumberOfBatchAllocations> {
public:
    freelist() noexcept :
