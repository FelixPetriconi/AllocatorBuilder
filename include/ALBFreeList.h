...
namespace ALB
{
   ...
   */
  template <bool Shared, class Allocator, int MinSize, int MaxSize, size_t PoolSize, 
    size_t NumberOfBatchAllocations>
  class FreeListBase {
    ...
    typename Traits::type_switch<
      ...
      >,
      Helper::stack<void*, PoolSize>,
      Shared
    >::type _root;

    Helper::Dynastic<
      (MinSize == DynamicSetSize? DynamicSetSize : MinSize), 
      DynamicSetSize> _lowerBound;
    Helper::Dynastic<
      (MaxSize == DynamicSetSize? DynamicSetSize : MaxSize), 
      ...
      if (_lowerBound.value() <= n && n <= _upperBound.value()) {
        ...
        if (_root.pop(freeBlock)) {
          ...
          if (supports_truncated_deallocation) {
            ...
            auto batchAllocatedBlocks = _allocator.allocate(blockSize * 
            ...
            if (batchAllocatedBlocks) {
              ...
              for (size_t i = 1; i < NumberOfBatchAllocations; i++) {
                if (!_root.push(static_cast<char*>(batchAllocatedBlocks.ptr) + 
                  i * blockSize)) {
                  BOOST_ASSERT(false);
                  _allocator.deallocate(Block(static_cast<char*>(batchAllocatedBlocks.ptr) 
  ...
  class SharedFreeList : 
    public FreeListBase<true, Allocator, MinSize, MaxSize, PoolSize, NumberOfBatchAllocations> {
  public:
  ...
  /**
  * This class is a single threaded specialization of the FreeList. For details see
  * ALB::FreeListBase
  ...
  */
  template <class Allocator, int MinSize, int MaxSize, size_t PoolSize = 1024,
    size_t NumberOfBatchAllocations = 8>
  class FreeList :
    public FreeListBase<false, Allocator, MinSize, MaxSize, PoolSize, NumberOfBatchAllocations> {
  public:
